...
export default function Projects() {
  ...
  return (
    <section id="projects" className="py-16 sm:py-24 bg-[#1e293b] relative overflow-hidden">
      <div className="absolute top-20 left-1/4 w-80 h-80 bg-gold/5 rounded-full blur-3xl hidden sm:block" />
      <div ref={ref} className="mx-auto max-w-6xl px-5 sm:px-10 relative">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={v ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <p className="text-cyan font-semibold text-sm mb-2 tracking-wide uppercase">What I Built</p>
        ...
        <motion.div
          ...
          <div className="absolute top-0 right-0 w-24 sm:w-32 h-24 sm:h-32 bg-gradient-to-bl from-cyan/5 to-transparent rounded-bl-full" />

          <div className="relative">
            <motion.span initial={{ opacity: 0, scale: 0.8 }} animate={v ? { opacity: 1, scale: 1 } : {}}
              transition={{ delay: 0.3 }}
              className="inline-block px-3 py-1 text-[11px] font-bold text-white bg-cyan rounded-full uppercase tracking-wider mb-4">
              Featured
            </motion.span>
            ...
            <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-6 sm:mb-8">
              {tech.map((t, i) => (
                <motion.span key={t}
                  ...
                  className="px-2 sm:px-3 py-1 sm:py-1.5 text-[11px] sm:text-sm lg:text-base font-semibold text-cyan-force bg-cyan-light rounded-lg hover:bg-cyan transition-all duration-200 cursor-default">
                  {t}
                </motion.span>
              ))}
            </div>
            ...
            <div className="space-y-2 sm:space-y-3">
              {points.map((p, i) => (
                <motion.div key={i}
                  ...
                  <span className="text-cyan font-bold shrink-0 mt-0.5">{String(i + 1).padStart(2, "0")}</span>
                  <span>{p}</span>
                </motion.div>
              ))}
            </div>
