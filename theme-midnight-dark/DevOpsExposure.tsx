...
export default function DevOpsExposure() {
  ...
  return (
    <section className="py-16 sm:py-24 bg-base relative overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_30%,rgba(6,182,212,0.04),transparent_50%)]" />
      ...
      <div ref={ref} className="mx-auto max-w-6xl px-5 sm:px-10 relative">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={v ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <p className="text-cyan font-semibold text-sm mb-2 tracking-wide uppercase">Hands-On</p>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          {items.map((item, i) => (
            <motion.div key={item.title}
              ...
              className={`rounded-xl p-4 sm:p-6 bg-[#1e293b] transition-all duration-300 cursor-default shadow-sm hover:shadow-xl ${
                item.highlight
                  ? "border-l-4 border-l-cyan border border-cyan/10"
                  : "border border-border hover:border-cyan/20"
              }`}>
