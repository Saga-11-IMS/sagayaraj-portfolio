...
export default function Experience() {
  ...
  return (
    <section id="experience" className="py-16 sm:py-24 bg-[#1e293b] relative overflow-hidden">
      <div className="absolute top-0 right-0 w-72 h-72 bg-cyan/3 rounded-full blur-3xl hidden sm:block" />
      <div className="absolute bottom-0 left-0 w-72 h-72 bg-gold/5 rounded-full blur-3xl hidden sm:block" />
      ...
      <div ref={ref} className="mx-auto max-w-6xl px-5 sm:px-10 relative">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={v ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <p className="text-cyan font-semibold text-sm mb-2 tracking-wide uppercase">Career Path</p>
          ...
          {jobs.map((job, i) => (
            <motion.div key={job.title + job.company}
              ...
              <div className={`absolute -left-[calc(1.25rem+5px)] sm:-left-[calc(2rem+5px)] top-2 w-3 h-3 rounded-full border-2 ${
                job.current ? "border-cyan bg-cyan" : "border-border-dark bg-[#1e293b]"
              }`} />
              {job.current && (
                <div className="absolute -left-[calc(1.25rem+5px)] sm:-left-[calc(2rem+5px)] top-2 w-3 h-3 rounded-full bg-cyan/40 animate-ping" />
              )}
              ...
              <motion.div
                ...
                className={`rounded-xl border p-4 sm:p-7 md:p-8 transition-all duration-300 ${
                  job.current
                    ? "border-cyan/20 bg-gradient-to-r from-cyan-light to-[#1e293b] shadow-md shadow-cyan/5"
                ...
                {job.current && (
                  <motion.span initial={{ opacity: 0, scale: 0.8 }} animate={v ? { opacity: 1, scale: 1 } : {}}
                    ...
                    className="inline-block mb-3 px-3 py-1 text-[11px] font-bold text-white bg-cyan rounded-full uppercase tracking-wider">
                    Current Role
                  </motion.span>
                )}
                <h3 className="dark-text text-lg sm:text-xl lg:text-2xl font-bold">{job.title}</h3>
                <p className="text-cyan font-semibold text-sm lg:text-base mt-1">{job.company}</p>
                ...
                <ul className="mt-4 sm:mt-5 space-y-2 sm:space-y-2.5">
                  {job.points.map((p, j) => (
                    <motion.li key={j}
                      ...
                      <span className="text-cyan mt-0.5 shrink-0 font-bold">▸</span>
                      <span>{p}</span>
                    </motion.li>
                  ))}
                </ul>
