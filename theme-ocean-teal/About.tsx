    ...
    <section id="about" className="py-16 sm:py-24 bg-white">
      <div ref={ref} className="mx-auto max-w-6xl px-5 sm:px-10">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={v ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <p className="text-teal font-semibold text-sm mb-2 tracking-wide uppercase">About Me</p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={v ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.1 }}
          ...
          <p style={{ color: '#1f2937' }} className="leading-[1.9] text-[15px] sm:text-base lg:text-[17px] mb-4 sm:mb-5">
            System Engineer with <span style={{ color: '#111827' }} className="font-semibold">2+ years</span> of experience in Linux
          ...
          <p style={{ color: '#1f2937' }} className="leading-[1.9] text-[15px] sm:text-base lg:text-[17px]">
            Currently transitioning into <span className="text-teal font-semibold">DevOps</span>, with hands-on
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          {info.map((item, i) => (
            <motion.div key={item.label}
              ...
              className="flex items-start gap-3 sm:gap-4 p-4 sm:p-5 rounded-xl bg-base border border-border hover:border-teal/25 hover:shadow-lg hover:shadow-teal/5 transition-all duration-300">
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-teal-light flex items-center justify-center shrink-0">
