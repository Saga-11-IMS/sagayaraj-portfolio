    ...
    <section id="skills" className="py-16 sm:py-24 bg-base relative overflow-hidden">
      ...
      <div ref={ref} className="mx-auto max-w-6xl px-5 sm:px-10 relative">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={v ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <p className="text-indigo font-semibold text-sm mb-2 tracking-wide uppercase">My Toolkit</p>
          <h2 className="dark-text text-2xl sm:text-3xl lg:text-4xl font-bold mb-4">Skills & Technologies</h2>
          <div className="w-16 h-1 bg-amber rounded-full mb-10 sm:mb-14" />
        </motion.div>
              ...
              <button key={g.title} onClick={() => setActive(i)}
                className={`group flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl text-left transition-all duration-300 shrink-0 ${
                  active === i
                    ...
                    : "bg-white hover:bg-white/80 border border-border hover:border-indigo/20"
                }`}>
                <span className={`font-mono text-[10px] sm:text-xs font-bold tracking-wider ${active === i ? "text-amber" : "text-indigo/40"}`}>
                  {g.num}
                </span>
            ...
            <AnimatePresence mode="wait">
              <motion.div key={active} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.3 }}>
                <div className="flex items-baseline gap-3 sm:gap-4 mb-6 sm:mb-8">
                  <span className="text-4xl sm:text-6xl lg:text-7xl font-extrabold text-indigo/10 font-mono leading-none">{groups[active].num}</span>
                  <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                  {groups[active].skills.map((skill, j) => (
                    <motion.div key={skill} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }}
                      ...
                      className="group flex items-center gap-3 px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg bg-white border border-border hover:border-indigo/30 hover:bg-indigo-light transition-all duration-200">
                      <span className="w-2 h-2 rounded-full bg-indigo/30 group-hover:bg-indigo group-hover:scale-125 transition-all duration-200" />
