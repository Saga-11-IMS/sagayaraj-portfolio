...
export default function Contact() {
  ...
  return (
    <section id="contact" className="py-16 sm:py-24 bg-base relative overflow-hidden">
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-indigo/3 rounded-full blur-3xl hidden sm:block" />
      ...
      <div ref={ref} className="mx-auto max-w-6xl px-5 sm:px-10 relative">
        ...
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12">
            ...
            <div className="space-y-3 sm:space-y-4">
              {[
                ...
                <motion.div key={item.label}
                  ...
                  className="flex items-center gap-3 sm:gap-4 p-2.5 sm:p-3 rounded-lg hover:bg-white transition-colors">
                  ...
                  {item.href ? (
                    <a href={item.href} target={item.href.startsWith("http") ? "_blank" : undefined}
                      ...
                      className="text-[13px] sm:text-sm lg:text-base dark-text hover:text-indigo transition-colors font-medium">{item.label}</a>
          ...
          <motion.form onSubmit={submit}
            ...
            className="bg-white rounded-xl border border-border p-5 sm:p-7 space-y-4 sm:space-y-5 shadow-sm">
            <div>
              <label className="block text-sm lg:text-base font-semibold dark-text mb-2">Name</label>
              <input type="text" required placeholder="Your name"
                className="w-full bg-base border border-border rounded-lg dark-text px-4 py-3 text-sm lg:text-base placeholder-muted focus:outline-none focus:ring-2 focus:ring-indigo/20 focus:border-indigo transition-all" />
            </div>
            <div>
            ...
            <div>
              ...
              <textarea rows={4} required placeholder="Your message..."
                className="w-full bg-base border border-border rounded-lg dark-text px-4 py-3 text-sm lg:text-base placeholder-muted focus:outline-none focus:ring-2 focus:ring-indigo/20 focus:border-indigo transition-all resize-none" />
            </div>
            <motion.button type="submit"
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              className="w-full px-7 py-3.5 font-semibold text-sm lg:text-base text-white bg-indigo rounded-lg hover:bg-indigo-dark transition-colors shadow-lg shadow-indigo/25"
